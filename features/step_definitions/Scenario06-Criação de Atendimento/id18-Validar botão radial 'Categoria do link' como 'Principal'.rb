...
Dado('#0618: eu já tenha realizado meu cadastro previamente com '\
    'nome {string}, password {string} e permissao {string}') do |nome, password, permissao|
  RunStep.cadastrar(nome, password, permissao)
end

E('#0618: eu já tenha criado um Cliente, Site e Fornecedor previamente') do
  SetUp.cliente_principal
  SetUp.site_principal
  SetUp.fornecedor_principal
end

E('#0618: eu tiver passado o mouse sobre a aba Atendimentos') do
  @dashboard_page = DashboardPage.new
  @dashboard_page.cursor_hover('#atendimentos')
end

E('#0618: eu tiver clicado em {string}.') do |link_text|
  @dashboard_page.clicar_submenu_link(link_text)
end

Quando('#0618: eu escolher o item da lista Designação {string}') do |site|
...
E('#0618: eu escolher o botão radial de Categoria do link Principal') do
  @novo_atendimento_page.selecionar_radiobutton('#atendimento_link_1')
end

E('#0618: eu clicar no botão Enviar.') do
