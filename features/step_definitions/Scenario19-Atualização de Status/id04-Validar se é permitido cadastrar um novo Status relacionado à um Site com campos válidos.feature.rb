...
Dado('#1904: eu já tenha realizado meu cadastro previamente com '\
    'nome {string}, password {string} e permissao {string}') do |nome, password, permissao|
  SetUp.cadastrar(nome, password, permissao)
end

E('#1904: eu já tenha criado um Site e Status previamente') do
  SetUp.cliente('cliente_principal', 'XXXXXXXXXXXX01')

  SetUp.site('cliente_principal', 'RIR', 'site_principal', '10')
  SetUp.status('001s 002site_principal 003RIR', 'Pendente')
end

E('#1904: eu já tenha criado um Site com valores alternativos') do
  SetUp.cliente('cliente_alternativo', 'XXXXXXXXXXXX02')
  SetUp.site('cliente_alternativo', 'RIR2', 'site_alternativo', '20')
end

E('#1904: eu tiver passado o mouse sobre a aba Status') do
  @dashboard_page = DashboardPage.new
  @dashboard_page.cursor_hover('#statuses')
end

E('#1904: eu tenha clicado no link {string}') do |link_text|
  @dashboard_page.clicar_submenu_link(link_text)
end

E('#1904: eu tenha clicado no link Editar do Status criado previamente.') do
  @listar_todos_page = ListarTodosPage.new('statuses')
  @listar_todos_page.clicar_link_editar
end

Quando('#1904: eu escolher o item da Originário {string}') do |item|
