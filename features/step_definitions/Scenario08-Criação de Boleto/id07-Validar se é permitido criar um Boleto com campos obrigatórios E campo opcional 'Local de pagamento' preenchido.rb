...
Dado('#0807: eu já tenha realizado meu cadastro previamente com '\
    'nome {string}, password {string} e permissao {string}') do |nome, password, permissao|
  SetUp.cadastrar(nome, password, permissao)
end

E('#0807: eu já tenha criado um Cliente, Site, Fornecedor, Atendimento, Status e Fatura previamente') do
...
E('#0807: eu tiver passado o mouse sobre a aba {string}') do |aba|
  @dashboard_page = DashboardPage.new
  @dashboard_page.cursor_hover("##{aba}")
end

E('#0807: eu tiver clicado em {string}.') do |link_text|
  @dashboard_page.clicar_submenu_link(link_text)
end

Quando('#0807: eu escolher o item da lista Atendimento {string}') do |item|
  @boleto_page = FormPage.new('boletos')
  @boleto_page.selecionar_item('#boleto_codigo_atendimento_composto', item)
end

E('#0807: eu escolher o item da lista Fatura {string}') do |item|
  @boleto_page.selecionar_item('#boleto_codigo_fatura', item)
end

E('#0807: eu escolher o item da lista Agrupamento {string}') do |item|
  @boleto_page.selecionar_item('#boleto_descricao_grupo', item)
end

E('#0807: eu escolher o item da lista Status {string}') do |item|
  @boleto_page.selecionar_item('#boleto_codigo_status', item)
end

E('#0807: eu escolher o item da lista data de vencimento {string}') do |item|
  @boleto_page.selecionar_item('#boleto_vencimento', item)
end

E('#0807: eu escolher o item da lista Local de pagamento {string}') do |item|
  @boleto_page.selecionar_item('#boleto_codigo_local_pagamento', item)
end

E('#0807: eu clicar no botão Enviar.') do
  @boleto_page.clicar_enviar
end

Então('#0807: eu sou redirecionado para detalhes do novo Boleto criado contendo valores informados e valores autopreenchidos') do
...
E('#0807: eu vejo a informação de que o Boleto foi criado com sucesso com o valor de codigo_boleto_composto {string}.') do |mensagem|
