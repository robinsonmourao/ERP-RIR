...
Given('#CADASTRAMENTO_TO_DESCADASTRAMENTO_LENIENT_CONTEXT: '\
...
When("clico em Fechar minha conta") do
  @visao_geral_conta_page.clicar_deletar_conta_link
end
And("preencha o campo com minha atual password {string}.") do |password|
  @visao_geral_conta_page.preencher_dados(password)
  @visao_geral_conta_page.clicar_fechar_conta
end

Then('sou redirecionado para tela entrar.') do
  expect(page).to have_current_path('http://127.0.0.1:3000/', url: true)
end
And('#MESSAGE_TO_DESCADASTRAMENTO_LENIENT_CONTEXT: vejo uma mensagem de confirmação de remoção de conta com nome {string}') do |nome|
