...
E('#0705: eu tiver passado o mouse sobre a aba {string}') do |endpoint|
  @dashboard_page = DashboardPage.new
  @dashboard_page.cursor_hover("##{endpoint.downcase}")
end

E('#0705: eu tenha clicado no link {string}') do |link_text|
  @dashboard_page.clicar_submenu_link(link_text)
end

E('#0705: eu tenha clicado no link Editar do Atendimento criado previamente.') do
...
E('#0705: eu clicar no botão Enviar.') do
  @form_atendimento_page.clicar_enviar
end

Então('#0705: eu vejo uma mensagem informando que a velocidade de download deve ser maior que "0"') do
