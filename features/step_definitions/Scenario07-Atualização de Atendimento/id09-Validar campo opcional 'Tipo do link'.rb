...
Dado('#0709: eu já tenha realizado meu cadastro previamente com '\
    'nome {string}, password {string} e permissao {string}') do |nome, password, permissao|
  SetUp.cadastrar(nome, password, permissao)
end

E('#0709: eu já tenha criado um Cliente, Site, Fornecedor e Atendimento previamente') do
...
E('#0709: eu tiver passado o mouse sobre a aba {string}') do |endpoint|
  @dashboard_page = DashboardPage.new
  @dashboard_page.cursor_hover("##{endpoint.downcase}")
end

E('#0709: eu tenha clicado no link {string}') do |link_text|
  @dashboard_page.clicar_submenu_link(link_text)
end

E('#0709: eu tenha clicado no link Editar do Atendimento criado previamente.') do
...
E('#0709: eu clicar no botão Enviar.') do
  @form_atendimento_page.clicar_enviar
end

Então('#0709: eu sou redirecionado para detalhes do novo Atendimento criado contendo valores informados') do
...
E('#0709: eu vejo a informação de que o Atendimento foi criado com sucesso com o valor de codigo_atendimento_composto em formato <001designação 002nome_do_fornecedor>.') do
